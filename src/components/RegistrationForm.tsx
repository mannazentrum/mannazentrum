...
const labelClass = "text-xs font-black uppercase text-brand-brown/60 mb-2 block tracking-widest ml-1";
...
const GuardianInputGroup = React.memo(({ 
  ...
  return (
    <div className="space-y-6 p-8 bg-brand-brown/5 rounded-[50px] border border-brand-brown/5 shadow-inner">
      <h4 className="text-brand-brown font-black uppercase tracking-widest text-sm border-b-2 border-brand-brown/10 pb-2">{title}</h4>
  ...
  const renderParentStep = () => {
    switch (step) {
      case 1: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight">Langkah 1: Identitas Anak</h3>
            <div className="grid md:grid-cols-2 gap-6">
      ...
      case 2: 
        return (
          <div className="space-y-10 animate-fadeIn">
            <div>
              <h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight">Langkah 2: Data Wali / Orang Tua</h3>
              <p className="text-[10px] font-black text-brand-brown/40 uppercase tracking-widest mt-1">Lengkapi data Ayah dan Bunda secara terpisah (Sama-sama Wali)</p>
            </div>
      ...
      case 3: 
        return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight leading-none mb-2">Langkah 3: Riwayat & Kebiasaan</h3><p className="text-xs font-bold text-brand-brown/40 uppercase tracking-widest">Informasi penting untuk kenyamanan anak di daycare</p></div>
      ...
      case 4: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight">Langkah 4: Kesepakatan & Tata Tertib</h3>
            <div className="h-96 overflow-y-auto p-8 bg-brand-brown text-white/90 rounded-[40px] text-sm leading-relaxed space-y-6 shadow-2xl border-4 border-white/10">
      ...
      case 5: 
         return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight leading-none mb-2">Langkah 5: Validasi Tanda Tangan</h3></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4"><span className="text-sm font-black text-brand-brown uppercase tracking-widest block text-center">Ayah / Wali</span><div className="relative border-2 border-brand-brown rounded-lg bg-white overflow-hidden shadow-inner"><canvas ref={canvasRefAyah} width={400} height={200} className="w-full h-48 cursor-crosshair touch-none"></canvas><button type="button" onClick={() => clearCanvas(canvasRefAyah)} className="absolute top-1 right-1 px-2 text-sm rounded border-2 border-brand-brown text-brand-brown hover:bg-gray-100 transition-all">Reset</button></div></div>
              <div className="space-y-4"><span className="text-sm font-black text-brand-brown uppercase tracking-widest block text-center">Bunda / Wali</span><div className="relative border-2 border-brand-brown rounded-lg bg-white overflow-hidden shadow-inner"><canvas ref={canvasRefIbu} width={400} height={200} className="w-full h-48 cursor-crosshair touch-none"></canvas><button type="button" onClick={() => clearCanvas(canvasRefIbu)} className="absolute top-1 right-1 px-2 text-sm rounded border-2 border-brand-brown text-brand-brown hover:bg-gray-100 transition-all">Reset</button></div></div>
            </div>
          </div>
         );
      case 6: 
        return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-brand-brown uppercase tracking-tight leading-none mb-2">Langkah 6: Pengaturan Akun Portal</h3></div>
    ...
    return (
      <div className="min-h-screen bg-brand-cream p-6 flex flex-col items-center justify-center">
        <div className="max-w-xl w-full bg-white rounded-[60px] shadow-2xl p-16 text-center border-4 border-brand-brown animate-fadeIn">
          ...
          <p className="text-lg text-brand-brown/60 mb-10 font-bold">Data pendaftaran telah diarsipkan ke Cloud. Admin akan mereview dalam 1x24 jam.</p>
    ...
    return (
      <div className="min-h-screen bg-brand-cream p-6 flex flex-col items-center justify-center animate-fadeIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-5xl">
          ...
          <button onClick={() => setRegType('teacher')} className="bg-white p-16 rounded-[80px] shadow-2xl border-4 border-transparent hover:border-brand-brown transition-all group flex flex-col items-center transform hover:-translate-y-4"><span className="text-7xl mb-10">👩‍🏫</span><span className="text-3xl font-black text-brand-brown uppercase text-center">Registrasi <br/> Staff / Guru</span></button>
        </div>
        <button onClick={onBack} className="mt-20 text-brand-brown/40 font-black hover:text-brand-brown uppercase tracking-[0.3em] text-[10px] border-b-2 border-transparent hover:border-brand-brown pb-1">Batal</button>
      </div>
    );
  }
