...
const inputClass = "p-2 rounded-lg border-2 border-dynamic-primary text-dynamic-primary placeholder:text-dynamic-primary placeholder:opacity-60 bg-transparent w-full";
const primaryBtnClass = "w-full bg-dynamic-primary text-white p-3 rounded-full mt-6 font-bold uppercase tracking-wider shadow-md hover:bg-black transition-all active:scale-95 border-b-4 border-black/30";
const secondaryBtnClass = "w-1/2 bg-white text-dynamic-primary border-2 border-dynamic-primary p-3 rounded-full font-bold uppercase tracking-wider shadow-md hover:bg-gray-100 transition-all active:scale-95";
const labelClass = "text-xs font-black uppercase text-dynamic-primary/60 mb-2 block tracking-widest ml-1";
...
const GuardianInputGroup = React.memo(({ 
  ...
  return (
    <div className="space-y-6 p-8 bg-dynamic-primary/5 rounded-[50px] border border-dynamic-primary/5 shadow-inner">
      <h4 className="text-dynamic-primary font-black uppercase tracking-widest text-sm border-b-2 border-dynamic-primary/10 pb-2">{title}</h4>
...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onBack, onBackToLanding, onRegister }) => {
  ...
  const renderParentStep = () => {
    switch (step) {
      case 1: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight">Langkah 1: Identitas Anak</h3>
            <div className="grid md:grid-cols-2 gap-6">
      ...
      case 2: 
        return (
          <div className="space-y-10 animate-fadeIn">
            <div>
              <h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight">Langkah 2: Data Wali / Orang Tua</h3>
              <p className="text-[10px] font-black text-dynamic-primary/40 uppercase tracking-widest mt-1">Lengkapi data Ayah dan Bunda secara terpisah (Sama-sama Wali)</p>
            </div>
      ...
      case 3: 
        return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight leading-none mb-2">Langkah 3: Riwayat & Kebiasaan</h3><p className="text-xs font-bold text-dynamic-primary/40 uppercase tracking-widest">Informasi penting untuk kenyamanan anak di daycare</p></div>
      ...
      case 4: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight">Langkah 4: Kesepakatan & Tata Tertib</h3>
            <div className="h-96 overflow-y-auto p-8 bg-dynamic-primary text-white/90 rounded-[40px] text-sm leading-relaxed space-y-6 shadow-2xl border-4 border-white/10">
      ...
      case 5: 
         return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight leading-none mb-2">Langkah 5: Validasi Tanda Tangan</h3></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4"><span className="text-sm font-black text-dynamic-primary uppercase tracking-widest block text-center">Ayah / Wali</span><div className="relative border-2 border-dynamic-primary rounded-lg bg-white overflow-hidden shadow-inner"><canvas ref={canvasRefAyah} width={400} height={200} className="w-full h-48 cursor-crosshair touch-none"></canvas><button type="button" onClick={() => clearCanvas(canvasRefAyah)} className="absolute top-1 right-1 px-2 text-sm rounded border-2 border-dynamic-primary text-dynamic-primary hover:bg-gray-100 transition-all">Reset</button></div></div>
              <div className="space-y-4"><span className="text-sm font-black text-dynamic-primary uppercase tracking-widest block text-center">Bunda / Wali</span><div className="relative border-2 border-dynamic-primary rounded-lg bg-white overflow-hidden shadow-inner"><canvas ref={canvasRefIbu} width={400} height={200} className="w-full h-48 cursor-crosshair touch-none"></canvas><button type="button" onClick={() => clearCanvas(canvasRefIbu)} className="absolute top-1 right-1 px-2 text-sm rounded border-2 border-dynamic-primary text-dynamic-primary hover:bg-gray-100 transition-all">Reset</button></div></div>
            </div>
          </div>
         );
      case 6: 
        return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-dynamic-primary uppercase tracking-tight leading-none mb-2">Langkah 6: Pengaturan Akun Portal</h3></div>
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center justify-center">
        <div className="max-w-xl w-full bg-white rounded-[60px] shadow-2xl p-16 text-center border-4 border-dynamic-primary animate-fadeIn">
          ...
          <h3 className="text-4xl font-black font-rounded text-dynamic-primary mb-6 uppercase tracking-tighter">Pendaftaran Selesai!</h3>
          <p className="text-lg text-dynamic-primary/60 mb-10 font-bold">Data pendaftaran telah diarsipkan ke Cloud. Admin akan mereview dalam 1x24 jam.</p>
  ...
  if (!regType) {
    return (
      <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center justify-center animate-fadeIn">
        <h2 className="text-5xl font-black font-rounded text-dynamic-primary mb-20 uppercase tracking-tighter text-center">Daftar Akun Baru</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-5xl">
          <button onClick={() => setRegType('parent')} className="bg-white p-16 rounded-[80px] shadow-2xl border-4 border-transparent hover:border-dynamic-primary transition-all group flex flex-col items-center transform hover:-translate-y-4"><span className="text-7xl mb-10">👨‍👩‍👧</span><span className="text-3xl font-black text-dynamic-primary uppercase text-center">Registrasi <br/> Orang Tua</span></button>
          <button onClick={() => setRegType('teacher')} className="bg-white p-16 rounded-[80px] shadow-2xl border-4 border-transparent hover:border-dynamic-primary transition-all group flex flex-col items-center transform hover:-translate-y-4"><span className="text-7xl mb-10">👩‍🏫</span><span className="text-3xl font-black text-dynamic-primary uppercase text-center">Registrasi <br/> Staff / Guru</span></button>
        </div>
        <button onClick={onBack} className="mt-20 text-dynamic-primary/40 font-black hover:text-dynamic-primary uppercase tracking-[0.3em] text-[10px] border-b-2 border-transparent hover:border-dynamic-primary pb-1">Batal</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center overflow-y-auto">
      <div className="w-full max-w-5xl bg-white rounded-[70px] shadow-2xl p-10 md:p-20 border-2 border-dynamic-primary/5 my-10 relative">
        <div className="flex items-center justify-between mb-16">
          <button onClick={() => setRegType(null)} className="w-14 h-14 bg-pdf-cream text-dynamic-primary rounded-full flex items-center justify-center text-3xl hover:scale-110">←</button>
          <div className="text-center"><h2 className="text-3xl font-black font-rounded text-dynamic-primary uppercase tracking-tighter">{regType === 'parent' ? 'Formulir Orang Tua' : 'Formulir Staf/Guru'}</h2><p className="text-[10px] font-black text-dynamic-primary/40 uppercase tracking-[0.4em] mt-2">Mannazentrum Cloud Sync</p></div>
          {regType === 'parent' && <div className="flex flex-col items-end"><span className="text-[10px] font-black uppercase text-dynamic-primary/30">Progress</span><span className="bg-dynamic-primary text-white px-6 py-2 rounded-full text-xs font-black uppercase shadow-lg mt-1">{step} / 6</span></div>}
        </div>
