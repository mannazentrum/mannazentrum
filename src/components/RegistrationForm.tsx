...
const GuardianInputGroup = React.memo(({ 
  ...
  return (
    <div className="space-y-6 p-8 bg-pdf-cream/30 rounded-[50px] border border-pdf-brown/5 shadow-inner">
      <h4 className="text-pdf-brown font-black uppercase tracking-widest text-sm border-b-2 border-pdf-brown/10 pb-2">{title}</h4>
...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onBack, onBackToLanding, onRegister }) => {
  ...
  const renderParentStep = () => {
    switch (step) {
      case 1: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-pdf-brown uppercase tracking-tight">Langkah 1: Identitas Anak</h3>
            <div className="grid md:grid-cols-2 gap-6">
      ...
      case 2: 
        return (
          <div className="space-y-10 animate-fadeIn">
            <div>
              <h3 className="text-2xl font-black font-rounded text-pdf-brown uppercase tracking-tight">Langkah 2: Data Wali / Orang Tua</h3>
              <p className="text-[10px] font-black text-pdf-brown/40 uppercase tracking-widest mt-1">Lengkapi data Ayah dan Bunda secara terpisah (Sama-sama Wali)</p>
            </div>
      ...
      case 4: 
        return (
          <div className="space-y-6 animate-fadeIn">
            <h3 className="text-2xl font-black font-rounded text-pdf-brown uppercase tracking-tight">Langkah 4: Kesepakatan & Tata Tertib</h3>
            <div className="h-96 overflow-y-auto p-8 bg-pdf-brown text-white/90 rounded-[40px] text-sm leading-relaxed space-y-6 shadow-2xl border-4 border-white/10">
      ...
      case 6: 
        return (
          <div className="space-y-8 animate-fadeIn">
            <div><h3 className="text-2xl font-black font-rounded text-pdf-brown uppercase tracking-tight leading-none mb-2">Langkah 6: Pengaturan Akun Portal</h3></div>
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center justify-center">
        <div className="max-w-xl w-full bg-white rounded-[60px] shadow-2xl p-16 text-center border-4 border-pdf-brown animate-fadeIn">
  ...
  if (!regType) {
    return (
      <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center justify-center animate-fadeIn">
        <h2 className="text-5xl font-black font-rounded text-pdf-brown mb-20 uppercase tracking-tighter text-center">Daftar Akun Baru</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-5xl">
          <button onClick={() => setRegType('parent')} className="bg-white p-16 rounded-[80px] shadow-2xl border-4 border-transparent hover:border-pdf-brown transition-all group flex flex-col items-center transform hover:-translate-y-4"><span className="text-7xl mb-10">👨‍👩‍👧</span><span className="text-3xl font-black text-pdf-brown uppercase text-center">Registrasi <br/> Orang Tua</span></button>
          <button onClick={() => setRegType('teacher')} className="bg-white p-16 rounded-[80px] shadow-2xl border-4 border-transparent hover:border-pdf-brown transition-all group flex flex-col items-center transform hover:-translate-y-4"><span className="text-7xl mb-10">👩‍🏫</span><span className="text-3xl font-black text-pdf-brown uppercase text-center">Registrasi <br/> Staff / Guru</span></button>
        </div>
        <button onClick={onBack} className="mt-20 text-pdf-brown/40 font-black hover:text-pdf-brown uppercase tracking-[0.3em] text-[10px] border-b-2 border-transparent hover:border-pdf-brown pb-1">Batal</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-pdf-cream p-6 flex flex-col items-center overflow-y-auto">
      <div className="w-full max-w-5xl bg-white rounded-[70px] shadow-2xl p-10 md:p-20 border-2 border-pdf-brown/5 my-10 relative">
        <div className="flex items-center justify-between mb-16">
          <button onClick={() => setRegType(null)} className="w-14 h-14 bg-pdf-cream text-pdf-brown rounded-full flex items-center justify-center text-3xl hover:scale-110">←</button>
          <div className="text-center"><h2 className="text-3xl font-black font-rounded text-pdf-brown uppercase tracking-tighter">{regType === 'parent' ? 'Formulir Orang Tua' : 'Formulir Staf/Guru'}</h2><p className="text-[10px] font-black text-pdf-brown/40 uppercase tracking-[0.4em] mt-2">Mannazentrum Cloud Sync</p></div>
          {regType === 'parent' && <div className="flex flex-col items-end"><span className="text-[10px] font-black uppercase text-pdf-brown/30">Progress</span><span className="bg-pdf-brown text-white px-6 py-2 rounded-full text-xs font-black uppercase shadow-lg mt-1">{step} / 6</span></div>}
        </div>
